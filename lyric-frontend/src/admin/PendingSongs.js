...
export default function PendingSongs() {
  ...
  const fetchPendingSongs = async () => {
    ...
    try {
      ...
      const { data } = await axios.get(`${config.lyric}/api/songs?status=pending`, {
  ...
  const handleAction = async (id, action, rejectionReason = '') => {
    const authToken = localStorage.getItem('adminAuthToken');
    try {
      await axios.patch(`${config.lyric}/api/songs/${id}/status`, { status: action, rejectionReason }, {
        headers: { 'x-auth-token': authToken }
      });
      fetchPendingSongs();
    } catch (err) {
  ...
  const handleBulkAction = async (action) => {
    ...
    const authToken = localStorage.getItem('adminAuthToken');
    try {
      await axios.patch(`${config.lyric}/api/songs/bulk-status`, { songIds: selected, status: action, rejectionReason: reason }, {
        headers: { 'x-auth-token': authToken }
      });
      setSelected([]);
      fetchPendingSongs();
